...
void walk_constructor(ast t)
{
    ...
    lcl_length = size_of_var_decs(get_subr_body_decs (subr_body) ) ;
    write_to_output ("function " + className + "." + name + " " + to_string(lcl_length) + '\n' ) ;
...
void walk_if_else(ast t)
{
    ...
    int thisIf = if_number;
    ++ if_number ;

    walk_expr(condition) ;
    write_to_output ("if-goto IF_TRUE" + to_string(thisIf) + "\ngoto IF_FALSE" + to_string(thisIf) + '\n' ) ;

    write_to_output ( "label IF_TRUE" + to_string(thisIf) + '\n' ) ;
    walk_statements(if_true) ;
    write_to_output ( "goto IF_END" + to_string(thisIf) + '\n' ) ;

    write_to_output ( "label IF_FALSE" + to_string(thisIf) + '\n' ) ;
    walk_statements(if_false) ;
    write_to_output ("label IF_END" + to_string(thisIf) + '\n' ) ;
}
...
void walk_while(ast t)
{
    ...
    int thisWhile = while_number ;
    ++while_number;
    write_to_output ("label WHILE_EXP" + to_string (thisWhile) + '\n' ) ;
    is_while = true;
    walk_expr(condition) ;
    is_while = false;
    write_to_output ("if-goto WHILE_END" + to_string (thisWhile) + '\n' ) ;
    walk_statements(body) ;
    write_to_output ("goto WHILE_EXP" + to_string (thisWhile) + "\nlabel WHILE_END" + to_string (thisWhile) + '\n' ) ;
}
