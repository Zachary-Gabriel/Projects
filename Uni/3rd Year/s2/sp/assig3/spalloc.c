...
void large_free(void *chunk) {
  ...
  munmap((void *)((uintptr_t)chunk - PAGESIZE), (pages + 2) << PAGESHIFT); /* this probably isn't a bug, but related to the "os = (*(uint64_t *)((uintptr_t)chunk - PAGESIZE)) << PAGESHIFT;" line, but I don't know what os means */
}
...
void *sp_calloc(size_t nmemb, size_t size) {
  ...
  printf ("{11}\n");
  bzero(rv, nmemb * size); // found using rnad_num 59, val was large malloc
  printf ("{12}\n");
  return rv;
}
