...
static int post_deliver_count = 0;
...
void post_deliver(void) {
	post_deliver_count += 1;
}
...
void handle_msg(void* data, size_t datasize, lp_msg_type type) {
    ...
    if(received[0] && received[1] && received[2]) {
		...
		if(pre_recv_count >= 2 && post_recv_count >= 2 && post_deliver_count >= 1) {
			printf("Pre/Post callbacks invoked correctly\n");
	        exit(0);
		}
...
int main (int argc, char const *argv[]) {
	...
	udp_receiver_set_postdeliver_callback(ur, post_deliver);
