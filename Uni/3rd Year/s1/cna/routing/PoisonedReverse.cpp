...
// executes the poisoned reverse part of the distance vector algorithm
void PR (Router*& network, Router*& duplicate, string* node_names, int node_num, int i, int j, int& time)
{
	// initialising variables
	...
	for (int k = 0; k < node_num; ++k)
	{
		...
		if (network[index].optimal[k] != -1 && i != k)
		{
			...
			{
				// poisoned reverse
				if (network[i].table[k][index] > duplicate[i].table[k][index] && duplicate[i].table[k][index] != -1)
				{
					network[i].table[k][index] = -1;
					duplicate[i].table[k][index] = -1;
					cout << "t="<< time <<" distance from "<< node_names[i] <<" to "<< node_names[k] << " via "<< node_names[index] << " is INF" << endl;
				}
				// normal DV
				else
				{
					network[i].table[k][index] = network[i].table[k][index];
			...
			if (network[i].table[k][index] != -1 && network[i].table[k][index] < min)
			{
...
void shuffle (int node_num, string* node_names, Router*& network)
{
	...
	while (change)
	{
		...
		for (int i = 0; i < node_num; ++i)
		{
			...
			{
				PR (network, duplicate, node_names, node_num, i, j, time);
			}
		}

		/* for testing */
		// print_variables_debug (node_num, node_names, network);
		// print_variables_debug (node_num, node_names, duplicate);
